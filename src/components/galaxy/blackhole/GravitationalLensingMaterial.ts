...
export const createGravitationalLensingMaterial = () => {
  return new THREE.ShaderMaterial({
    uniforms: {
      ...
      eventHorizonRadius: { value: 0.06 }, // Increased slightly to compensate for smaller plane
      distortionRadius: { value: 0.08 }, // Increased slightly to compensate for smaller plane
    },
    vertexShader: `
      varying vec2 vUv;
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        if (distFromCenter > eventHorizonRadius && distFromCenter < distortionRadius) {
          ...
          // Multiply the layers together for complex interaction
          float combinedAlpha = layer1Alpha + layer2Alpha + layer3Alpha;
          float multiplicativeEffect = layer1Alpha * layer2Alpha * layer3Alpha * 5.0; // Boost the multiply effect
